...
static void fill_table_4_bit(int NrOfChannels, int NrOfBitsPerCh, 
                          segment_t *S, int **Table4Bit)
{
  int BitNr;
  int ChNr;
  int SegNr;
...
static int reverse_7_lsbs(int c)
{
  int LSBs;
  int i;
  int p;
...
int dst_frame_decode(unsigned char*    DSTdata, 
                      ...
                      ebunch_t*           D)
{
  ...
  unpack_dst_frame(D, DSTdata, MuxedDSDdata);
  ...
  if (D->FrameHdr.DSTCoded == 1)
  {
    ...
    fill_table_4_bit(D->FrameHdr.NrOfChannels, D->FrameHdr.NrOfBitsPerCh, 
                  &D->FrameHdr.FSeg, D->FrameHdr.Filter4Bit);
    fill_table_4_bit(D->FrameHdr.NrOfChannels, D->FrameHdr.NrOfBitsPerCh, 
                  &D->FrameHdr.PSeg, D->FrameHdr.Ptable4Bit);
  
    D->DstXbits.PBit = reverse_7_lsbs(D->FrameHdr.ICoefA[0][0]);
  
    dst_ac_decode_bit(&D->DstXbits.Bit, D->DstXbits.PBit, D->AData,  D->ADataLen, 0);
    ...
    for(BitNr = 0; BitNr < D->FrameHdr.NrOfBitsPerCh; BitNr++)
    {
      for(ChNr = 0; ChNr < D->FrameHdr.NrOfChannels; ChNr++)
      {
        ...
        {
          dst_ac_decode_bit(&(D->BitResidual[ChNr][BitNr]),
                          AC_PROBS / 2, D->AData,  D->ADataLen, 0);
        }
        else
        {
          PtableIndex = dst_ac_get_ptable_index(D->PredicVal[ChNr][BitNr],
    ...
    dst_ac_decode_bit(&ACError, 0, D->AData, D->ADataLen, 1);
...
/*                                                                         */
/***************************************************************************/
ebunch_t * open_dst_decoder(int8_t channel_count) {
	ebunch_t * dst_decoder = (ebunch_t *) malloc(sizeof(ebunch_t));
	memset(dst_decoder, 0, sizeof(ebunch_t));

	dst_decoder->FrameHdr.NrOfChannels = channel_count;

	dst_decoder->FrameHdr.FrameNr = 0;
	dst_decoder->StrFilter.TableType = FILTER;
	dst_decoder->StrPtable.TableType = PTABLE;
	dst_decoder->FrameHdr.MaxFrameLen = (588 * 64 / 8); 
	dst_decoder->FrameHdr.ByteStreamLen   = dst_decoder->FrameHdr.MaxFrameLen   * dst_decoder->FrameHdr.NrOfChannels;
	dst_decoder->FrameHdr.BitStreamLen    = dst_decoder->FrameHdr.ByteStreamLen * RESOL;
	dst_decoder->FrameHdr.NrOfBitsPerCh   = dst_decoder->FrameHdr.MaxFrameLen   * RESOL;
	dst_decoder->FrameHdr.MaxNrOfFilters  = 2 * dst_decoder->FrameHdr.NrOfChannels;
	dst_decoder->FrameHdr.MaxNrOfPtables  = 2 * dst_decoder->FrameHdr.NrOfChannels;

	allocate_decoder_memory(dst_decoder);
	ccp_calc_init(&dst_decoder->StrFilter);
	ccp_calc_init(&dst_decoder->StrPtable);
...
/*                                                                         */
/***************************************************************************/
void close_dst_decoder(ebunch_t * dst_decoder) {

	/* Free the decoder memory */
	free_decoder_memory(dst_decoder);
